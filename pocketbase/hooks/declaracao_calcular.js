routerAdd(
  ...
  (e) => {
    ...
    if (!ano) {
      return e.badRequestError('Ano-calendário não definido na declaração')
    }
    ...
    try {
      ...
      return e.badRequestError(
        'Não existe tabela progressiva cadastrada para o ano ' +
          ano +
    ...
    if (!Array.isArray(faixas) || faixas.length === 0) {
      return e.badRequestError(
        'A tabela progressiva do ano ' + ano + ' não possui faixas configuradas.',
      )
    }
    ...
    var rends = []
    try {
      rends = $app.findRecordsByFilter('rendimentos', 'declaracao_id = {:id}', 'created', 500, 0, {
        id: decId,
      })
    } catch (_) {}
